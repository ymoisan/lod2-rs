...
impl BuildingPointIndex {
    pub fn from_laz(path: &Path) -> Result<Self> {
        ...
        let mut reader =
            las::Reader::from_path(path).with_context(|| format!("opening {}", path.display()))?;
        ...
        let total_class6 = pts.len();
        info!(
            "{total} points total, {total_class6} class-6 (building)"
        );

        let tree = RTree::bulk_load(pts);
        Ok(Self { tree, total_class6 })
    }

    pub fn total_class6(&self) -> usize {
        self.total_class6
    }
    ...
    /// Return the class-6 points inside a bounding box.
    pub fn points_in_bbox(&self, min_x: f64, min_y: f64, max_x: f64, max_y: f64) -> Vec<[f64; 2]> {
        let envelope = AABB::from_corners([min_x, min_y], [max_x, max_y]);
        self.tree
            .locate_in_envelope(&envelope)
            .copied()
...
pub fn bbox(verts: &[(f64, f64)]) -> (f64, f64, f64, f64) {
...
pub fn point_in_polygon(px: f64, py: f64, verts: &[(f64, f64)]) -> bool {
    let n = verts.len();
    if n < 3 {
        return false;
    }
