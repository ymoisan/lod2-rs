...
/// Compute the minimum-area bounding rectangle (MABR) of a set of points.
/// Returns the 4 corner points of the rectangle.
pub fn min_area_bounding_rect(points: &[(f64, f64)]) -> Vec<(f64, f64)> {
...
fn min_area_bounding_rect_area(points: &[(f64, f64)]) -> f64 {
    let corners = min_area_bounding_rect(points);
    if corners.len() < 4 {
        return 0.0;
    }
    let dx1 = corners[1].0 - corners[0].0;
    let dy1 = corners[1].1 - corners[0].1;
    let dx2 = corners[3].0 - corners[0].0;
    let dy2 = corners[3].1 - corners[0].1;
